...
class ThreadManagementExercisesTests extends BaseTestSuite {
  test("ThreadManagementExercises.ex1") {
    ...
    val f = ThreadManagementExercisesSolutions.ex1(task).attempt.runToFuture
  ...
  test("ThreadManagementExercises.ex2") {
    ...
    val f = ThreadManagementExercisesSolutions.ex2(normalTask, blockingTask, s2).runToFuture
